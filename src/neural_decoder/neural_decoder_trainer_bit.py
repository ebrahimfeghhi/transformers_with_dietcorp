...
def trainModel(args):
    ...
    model = BiT(trial_size=(X.shape[1], X.shape[2]), patch_size=(args['patch_height'], args['patch_width']), 
    num_classes=args['nClasses'], dim=args['embedding_dim'], depth=args['n_layers'], heads=args['n_heads'], 
    mlp_dim_ratio=args['mlp_ratio'], dropout=args['dropout']).to(args["device"])
